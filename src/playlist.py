...
class PlayList(YouTubeAPI):
    ...
    def total_duration(self):
        ...
        for item in playlist_videos["items"]:
            ...
            video_duration = self.youtube.videos().list(id=video_id, part="contentDetails"
                                                        ).execute()["items"][0]["contentDetails"]["duration"]
            duration_seconds = self.duration_to_seconds(video_duration)
            total_seconds += duration_seconds
        return datetime.timedelta(seconds=total_seconds)

    def show_best_video(self):
        """Находим видео с наибольшим количеством лайков в плейлисте"""
        playlist_videos = self.youtube.playlistItems().list(playlistId=self.playlist_id, part='snippet',
                                                            maxResults=50).execute()
        max_likes = 0
        best_video_url = ""
        for item in playlist_videos["items"]:
